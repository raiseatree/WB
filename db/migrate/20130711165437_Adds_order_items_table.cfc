...
--->
<cfcomponent extends="plugins.dbmigrate.Migration" hint="Adds order items table">
  <cffunction name="up">
    <cfscript>
      t = createTable(name='orderitems');
      
      t.references(referenceNames='order', foreignKey=false);
      t.string(columnNames="item", null=false);
      t.integer(columnNames="quantity");
      t.string(columnNames="notes", null=true);
